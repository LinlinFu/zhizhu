...
@property (nonatomic, strong) UIImageView *imageView; //占位图片

@property (nonatomic, assign) BOOL *isFirstLoad; //是否是首次加载.....暂时不用
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.bigFrame = CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64);
...
- (void)setVideoImgUrl:(NSString *)videoImgUrl {
    _videoImgUrl = videoImgUrl;
    NSURL *photourl = [NSURL URLWithString:videoImgUrl];
    _imageView.image = [[UIImage alloc] initWithData:[NSData dataWithContentsOfURL:photourl]];
